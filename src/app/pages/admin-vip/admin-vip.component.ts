    ...
    <div class="page">
      ...
      <div class="card flush" style="margin-bottom:24px">
        ...
        <div class="table-wrap">
          <table class="tbl">
            <thead>
              <tr><th>ผู้เล่น</th><th>group_id</th><th>หมดอายุ</th><th>สถานะ</th><th></th></tr>
            </thead>
            <tbody>
              <tr *ngFor="let g of all?.items">
                ...
                <td class="mono" style="font-size:12px">{{ g.expires_at | date:'medium' }}</td>
      ...
      <div class="card" style="margin-bottom:16px">
        ...
        <div *ngIf="grantsLoaded" style="margin-top:16px">
          <table class="tbl" *ngIf="grants.length">
            <thead><tr><th>group_id</th><th>หมดอายุ</th><th>สถานะ</th><th></th></tr></thead>
            <tbody>
      ...
      <div *ngIf="settingExpiry" class="modal-backdrop">
        <div class="modal-card tactical" style="max-width:420px">
          ...
          <label><span class="muted" style="font-size:12px">วันหมดอายุ</span>
...
export class AdminVipComponent implements OnInit {
  ...
  openSet(g: VipGrant) {
    ...
    // prefill datetime-local with current expiry in the viewer's local time
    this.expiryLocal = isoToLocalInput(g.expires_at);
    this.error = null;
  }

  doSet() {
    if (!this.settingExpiry || !this.expiryLocal) { this.error = 'เลือกวันก่อน'; return; }
    const iso = localInputToIso(this.expiryLocal); // Thai wall-clock -> UTC ISO
