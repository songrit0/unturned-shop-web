      ...
      <div class="bg-gray-800/70 border border-gray-700 rounded-xl p-4 sm:p-5 mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
          <div class="sm:col-span-2">
            ...
            <input [(ngModel)]="form.title" maxlength="128" placeholder="e.g. How to sell items"
                   class="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 focus:border-yellow-500 outline-none" />
          </div>
          <div>
            <label class="block text-xs font-semibold tracking-wide text-gray-400 mb-1">CATEGORY</label>
            <input [(ngModel)]="form.category" maxlength="64" placeholder="e.g. Commands"
                   class="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 focus:border-yellow-500 outline-none" />
          </div>
        </div>

        <label class="block text-xs font-semibold tracking-wide text-gray-400 mb-1">BODY</label>
        <textarea [(ngModel)]="form.body" rows="6" placeholder="Explain the system / command here…"
                  class="w-full mb-3 px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 focus:border-yellow-500 outline-none font-mono text-sm"></textarea>

        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4 items-end">
          <div>
            <label class="block text-xs font-semibold tracking-wide text-gray-400 mb-1">ICON (1 char)</label>
            <input [(ngModel)]="form.icon" maxlength="4" placeholder="?"
                   class="w-full px-3 py-2 rounded-lg bg-gray-900 border border-gray-700 focus:border-yellow-500 outline-none text-center" />
          </div>
          <div>
            <label class="block text-xs font-semibold tracking-wide text-gray-400 mb-1">ORDER</label>
        ...
        <div class="flex items-center gap-3">
          ...
          <button *ngIf="editingId" (click)="cancelEdit()"
                  class="px-4 py-2 rounded-lg font-semibold border border-gray-600 text-gray-300">Cancel</button>
          <span *ngIf="msg" class="text-sm" [class.text-green-400]="!err" [class.text-red-400]="err">{{ msg }}</span>
        </div>
      </div>

      <!-- list -->
      <h2 class="text-sm font-semibold tracking-wide text-gray-400 mb-3">TOPICS</h2>
      <div class="space-y-2">
        <div *ngFor="let t of items"
          ...
          <span class="mt-0.5 w-7 h-7 flex-none rounded-lg bg-gray-900 border border-gray-700 grid place-items-center text-yellow-400 font-bold">{{ t.icon || '?' }}</span>
          <div class="flex-1 min-w-0">
            <div class="font-semibold">{{ t.title }}
              <span *ngIf="t.category" class="ml-2 text-xs px-2 py-0.5 rounded bg-gray-700 text-gray-300">{{ t.category }}</span>
              <span *ngIf="!t.enabled" class="ml-2 text-xs text-red-400">(hidden)</span>
            </div>
            <div class="text-sm text-gray-400 truncate">{{ t.body }}</div>
            <div class="text-xs text-gray-600 mt-0.5">order {{ t.sort_order }}</div>
          </div>
          <button (click)="edit(t)" class="text-yellow-400 text-sm hover:underline flex-none">Edit</button>
          <button (click)="remove(t)" class="text-red-400 text-sm hover:underline flex-none">Delete</button>
        </div>
